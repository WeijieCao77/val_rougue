...
import {RULES_VERSION,ROLES,TRAIT_TUNING,OPENING_FREE,OPENING_TRADE,GEAR,SUPPLIES,ENERGY_GEAR,SUPPLY_RARITY_WEIGHTS,SUPPLY_PRICES,GEAR_PRICES,BASE_SUPPLY_SLOTS,GEAR_SLOTS,GEAR_SELL,MAX_ASCENSION,ENEMY_TUNING,ENEMY_TUNING_V2,gearName,ECON_VERSION,GEAR_UNLOCKS,SKIP_FUNDS,REROLL_BASE,REROLL_STEP,INVESTMENTS} from './wa-rules.js';
...
export function regionUnlockPlan(region){
 return planCache[region]||=planCardUnlocks(REGIONS[region].pool,{start:REGIONS[region].start,rarityOf:id=>CARD_RARITY[id],groupOf:id=>TACTICS[id]?.archetype||null,salt:region});
}
// Cards that rewards, markets, events and the opening may offer in this season.
export function offerPool(s){
 if(!E(s))return REGIONS[s.region].pool;
 const k=`${s.region}:${s.unlockTier}`;
 return poolCache[k]||=unlockedFrom(regionUnlockPlan(s.region),REGIONS[s.region].pool,s.unlockTier);
}
...
export function restRate(s){return (R(s)&&s.ascension>=5?.2:.3)+(hasInvest(s,'IV02')?.1:0);}
