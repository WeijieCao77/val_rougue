...
function face(c,instance=false){const t=CARDS[c.id],f=TACTICS[c.id],tag=t.zone==='exhaust'?'消耗':t.zone==='temporary'?'临时 · 消耗':t.zone==='exhaustEnd'?'回合末消耗':t.zone==='power'?'持续能力':t.id.startsWith('CU')?'跨比赛保留':c.up?'已训练':t.player?'选手牌':'辅助牌';return `<span class="card-cost">${t.cost===null?'—':t.cost}</span><span class="card-title">${esc(cardName(c))}</span><span class="card-portrait" aria-hidden="true">${t.player?'<svg class="portrait-silhouette" viewBox="0 0 120 120" fill="currentColor"><circle cx="60" cy="38" r="22"/><path d="M17 120v-13c0-25 18-43 43-43s43 18 43 43v13z"/></svg>':icon(roleIcon[t.role]||'cards')}<span class="portrait-role">${esc(t.player?t.role:t.id.startsWith('CU')?'隐患':'行动')}</span>${t.player?`<span class="portrait-tier">${['黑铁','铜','银','金'][t.cost]}</span>`:''}</span><b class="card-tactic">${esc(f.title)}</b><span class="card-effect">${compactLines(c).map(line=>`<span>${esc(line).replace(/(\d+)/g,'<strong>$1</strong>')}</span>`).join('')}</span><span class="card-foot"><b class="${['exhaust','temporary','exhaustEnd'].includes(t.zone)?'exhaust-tag':''}">${tag}</b>${instance?' · '+c.uid:''}</span>`;}
...
function libraryEntry(id){const f=TACTICS[id];return `<div class="library-entry">${card({id,up:false},{upgrade:CARDS[id].player})}<details><summary>战术说明与出处</summary><p>${esc(f.note)}</p>${f.source?`<a href="${esc(f.source)}" target="_blank" rel="noopener noreferrer">查看参考来源</a>`:'<p>原创比赛／团队场景。</p>'}</details></div>`;}
