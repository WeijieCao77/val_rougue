...
const { WA_EVENTS, WA_EVENT_POOLS, WA_CRATE_LOOT } = module9;
...
const { CARD_RARITY, RARITY_ORDER } = module8;
...
return {clone,random,shuffle,instance,SEASON_EVENTS,offers,RARITY_ODDS,SHOP_PRICE_RANGE,rarityChances,rarityOffers,shopPrice,createRun,createSeason,startBattle,drawCards,damage,intent,intentText,canPlay,removalReason,healAmount,act,replay,preview,describeSeasonEvent,legalActions,observe};
})();
const module11=(()=>{
...
const module14=(()=>{
const { CARD_ART, ENEMY_ART } = module12;
const { weaponFrame } = module13;
const escape=x=>String(x??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
const { playSfx, soundForAction, soundForCard } = module16;
const { snapshotWa, diffCombat, playFeedback, cardSlam } = module17;
...
const module19=(()=>{
// combat-fx.js
const { waHit } = module18;
const { playSfx } = module16;
...
const module20=(()=>{
const { createSeason:createOriginalSeason, act:originalAct, clone, legalActions:originalLegalActions } = module10;
...
const module24=(()=>{
const { cardArtwork, opponentArtwork, artCredit } = module14;
const { combatEvents } = module15;
const { clearCombatFx, captureCombatStage, playCombatFx } = module19;
...
const { statusBadges, statusIcon, highlightKeywords } = module23;
const { createRun, canPlay, preview, intent, intentText, healAmount, removalReason, observe, shopPrice, describeSeasonEvent } = module10;
const { CARD_RARITY, RARITY_LABELS } = module8;
const { createWaSeason, waAct:act, waLegalActions:legalActions } = module20;
const { syncWaCheckpoint } = module21;
const { flyCardsFromPile, flyCardsToPile } = module22;
const { soundToggleHtml } = module16;
const { waJuiceAction, waSlam } = module18;
const createSeason=(seed,tutorial,region)=>createWaSeason(seed,tutorial,region,crypto.randomUUID());
const { routeNodes, mapEntry, nextScreen, restoreScreen } = module11;
