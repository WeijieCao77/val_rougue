...
return {RULES_VERSION,ROLES,TRAIT_TUNING,REGION_TRAITS,ASCENSION_LEVELS,MAX_ASCENSION,OPENING_OPTIONS,OPENING_FREE,OPENING_TRADE,RARITY,GEAR,ECON_VERSION,GEAR_UNLOCKS,SKIP_FUNDS,REROLL_BASE,REROLL_STEP,INVESTMENTS,ENERGY_GEAR,gearName,SUPPLY_RARITY_WEIGHTS,SUPPLIES,SUPPLY_PRICES,GEAR_PRICES,BASE_SUPPLY_SLOTS,GEAR_SLOTS,GEAR_SELL,ENEMY_TUNING,ENEMY_TUNING_V2};
})();
const module9=(()=>{
...
const { RULES_VERSION, ROLES, TRAIT_TUNING, OPENING_FREE, OPENING_TRADE, GEAR, SUPPLIES, ENERGY_GEAR, SUPPLY_RARITY_WEIGHTS, SUPPLY_PRICES, GEAR_PRICES, BASE_SUPPLY_SLOTS, GEAR_SLOTS, GEAR_SELL, MAX_ASCENSION, ENEMY_TUNING, ENEMY_TUNING_V2, gearName, ECON_VERSION, GEAR_UNLOCKS, SKIP_FUNDS, REROLL_BASE, REROLL_STEP, INVESTMENTS } = module8;
const { planCardUnlocks, unlockedFrom, tierOfId, validTier, UNLOCK_TIERS } = module11;
...
function regionUnlockPlan(region){
...
return {clone,random,shuffle,instance,SEASON_EVENTS,R,mapVersionOf,hasGear,E,hasInvest,regionUnlockPlan,offerPool,gearLocked,rerollPrice,supplySlots,marketPrice,rollGear,gearSellValue,restHeal,restRate,offers,RARITY_ODDS,SHOP_PRICE_RANGE,rarityChances,rarityOffers,shopPrice,createRun,createSeason,startBattle,drawCards,enemyMaxHp,damage,intent,intentText,incomingDamage,canPlay,removalReason,healAmount,act,replay,preview,describeSeasonEvent,legalActions,observe};
})();
const module13=(()=>{
...
const module16=(()=>{
const { CARD_ART, ENEMY_ART } = module14;
const { weaponFrame } = module15;
const escape=x=>String(x??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
const { CARDS, effects } = module7;
const { playSfx, soundForAction, soundForCard } = module18;
const { snapshotWa, diffCombat, playFeedback, cardSlam } = module19;
...
const module21=(()=>{
// combat-fx.js
const { waHit } = module20;
const { playSfx } = module18;
...
const module22=(()=>{
const { createSeason:createOriginalSeason, act:originalAct, clone, legalActions:originalLegalActions } = module12;
const { SKINS } = module7;
...
const module28=(()=>{
const { cardArtwork, opponentArtwork, artCredit } = module16;
const { combatEvents } = module17;
const { clearCombatFx, captureCombatStage, playCombatFx } = module21;
...
const { statusBadges, statusIcon, highlightKeywords } = module25;
const { createRun, canPlay, preview, intent, intentText, incomingDamage, healAmount, removalReason, observe, shopPrice, R, hasGear, supplySlots, marketPrice, restHeal, enemyMaxHp, gearSellValue, describeSeasonEvent, E, rerollPrice, restRate, regionUnlockPlan } = module12;
const { UNLOCK_TIERS, tierOfXp, runXp, normalizeProgress, progressTier, progressGearTier, awardRun } = module11;
const { CARD_RARITY, RARITY_LABELS } = module9;
const { REGION_TRAITS, TRAIT_TUNING, ASCENSION_LEVELS, MAX_ASCENSION, OPENING_OPTIONS, GEAR, SUPPLIES, RARITY, SUPPLY_PRICES, GEAR_PRICES, GEAR_SLOTS, gearName, ECON_VERSION, GEAR_UNLOCKS, SKIP_FUNDS, INVESTMENTS } = module8;
const { createWaSeason, waAct:act, waLegalActions:legalActions } = module22;
const { syncWaCheckpoint } = module23;
const { flyCardsFromPile, flyCardsToPile } = module24;
const { soundToggleHtml } = module18;
const { attachCardDetail, cardSheetOpen, showDragHint, hideDragHint, touchLift } = module27;
const { waJuiceAction, waSlam } = module20;
const { computeScore, scoreFormulaText, recordRun, loadHistory, markSeen, loadCollection, seenCount, trackStep, loadTracker, saveTracker, newTracker, filterSortCards, SORT_LABELS, COST_FILTERS, formatDuration } = module26;
...
const { routeNodes, mapEntry, nextScreen, restoreScreen } = module13;
